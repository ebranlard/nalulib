...
import pandas as pd
import numpy as np
from nalulib.weio.csv_file import CSVFile, find_non_numeric_header_lines
from nalulib.weio.plot3d_file import read_plot3d, write_plot3d
try:
...
def read_airfoil_csv_like(filename):
    ...
    if not np.issubdtype(x.dtype, np.floating) or not np.issubdtype(y.dtype, np.floating):
        if x[-1] == 'ZZ':
            ...
            print('First values of x:',x[0:5], x.dtype)
            print('First values of y:',y[0:5], y.dtype)
            raise BrokenFormatError("CSV file must contain floating point numbers in both columns. Maybe the header was not detected correctly?")
    d={}
    d['format'] = 'csv'
    return x, y, d
